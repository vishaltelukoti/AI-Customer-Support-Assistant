...
`backend/app/monitoring/mlflow_tracking.py` logs existing Day 3 classification and Day 6 retrieval metrics to a local file-based MLflow store. The compact tracked-run metadata is saved in `experiments/mlops/mlflow_tracking_result.json`, and the local configuration is saved in `experiments/mlops/mlflow_config.json`. The bulky local `experiments/mlflow/mlruns/` run store is ignored and was removed during final cleanup; it can be regenerated by rerunning the command.
